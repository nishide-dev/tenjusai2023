...
import { FadeInToUp, FadeInToRight, FadeInToLeft, FadeIn } from "../animations/FadeInAnimation";
type Event = {
    id: number;
    name: string;
    description: string;
...
const _events2: Event[] = [
    {
        id: 1,
        name: "天樹祭",
        description: "豊田工業大学 第40回大学祭",
        start: "2023-09-11T10:00:00.000Z",
        end: "2023-09-11T17:00:00.000Z",
        place: "豊田工業大学",
        image: "/next.svg",
        url: "https://tenjusai2023.vercel.app",
        day: 2,
                    ...
                    <div className="lg:w-full lg:max-w-7xl lg:px-8">
                        <FadeInToLeft>
                            ...
                            <p className="mt-6 text-xl leading-8 text-gray-600 mb-8">
                                当日の主なイベントを掲載しています。出演時間等変更になる場合がありますので、ご了承ください。詳しくはこちらの<Link href="/stage"><span className="text-tenjusaiOrange font-semibold after:content-['_↗'] hover:underline">ステージ</span></Link>、<Link href="/event"><span className="text-tenjusaiOrange font-semibold after:content-['_↗'] hover:underline">イベント</span></Link>、<Link href="/yatai"><span className="text-tenjusaiOrange font-semibold after:content-['_↗'] hover:underline">屋台</span></Link>のページをご覧ください。
                            </p>
                        </FadeInToLeft>
                        <FadeIn>
                        ...
                        <div className="grid border border-gray-300 rounded-lg shadow-lg sm:grid-cols-2 lg:grid-cols-3">
                            {events.map((event, index) => {
                                ...
                                return (
                                    <FadeInToUp key={index}>
                                        <figure key={index} className="border-b flex flex-col p-8 border-gray-300 md:rounded-t-none md:border-r">
                                            <blockquote className="max-w-2xl mb-4 text-gray-500 lg:mb-8">
                                                <span className="bg-green-100 text-green-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-full">{startMonth}/{startDay} {startHour} : {startMinutes} ~ {endHour} : {endMinutes}</span>
                                                <h3 className="text-lg font-semibold text-gray-900 mt-2">{event.name}</h3>
                                            </blockquote>
